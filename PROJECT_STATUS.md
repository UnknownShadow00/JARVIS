...
- 2026-07-02 remote session: all six Ollama env vars confirmed set at user scope and picked up by `ollama serve` (FLASH_ATTENTION=true, KV_CACHE_TYPE=q8_0, NUM_PARALLEL=2, NUM_BATCH=512, MAX_LOADED_MODELS=2, KEEP_ALIVE=-1) — the "requires Administrator shell" blocker no longer applies because user-scope vars cover a user-run Ollama. Live web search smoke passed with real DuckDuckGo results via `app.tools.web_search.execute`. Live vision smoke passed: qwen3-vl accurately described a live screen capture via `app.computer.vision.execute`. `npm audit` caught a new high-severity undici advisory in `frontend/electron`; fixed non-breaking via `npm audit fix` (lockfile-only change, 0 vulnerabilities after). `python tasks/pre_server_readiness.py` then passed all 6 checks including full pytest 350 passed.
