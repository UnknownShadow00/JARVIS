...
def execute(params: dict[str, Any]) -> list[dict[str, str]] | str | dict[str, str]:
    """Search the web, or fetch a page when action='fetch'."""
    if settings.safety.dry_run:
        return f"[DRY RUN] Would execute web_search with params {params}"
    ...
    try:
        ...
        try:
            ...
            return {"error": "DuckDuckGo search dependency is not installed."}
...
def fetch_page(url: str) -> str | dict[str, str]:
